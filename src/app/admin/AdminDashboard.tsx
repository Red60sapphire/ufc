...
function ReplaysTab({ replays, onRefresh }: { replays: Replay[]; onRefresh: () => void }) {
  ...
  const handleScrape = async () => {
    ...
    try {
      ...
      const data = await res.json();
      if (data.success) {
        setScrapeResult(`Found ${data.events} events, ${data.fights} fights, added ${data.newFights} new. ${data.errors?.length || 0} errors.`);
        onRefresh();
      } else {
    ...
    <div className="space-y-4">
      <div className="flex flex-col sm:flex-row gap-3 items-start sm:items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <button onClick={handleScrape} disabled={scraping} className="bg-emerald-600 text-white px-4 py-2 text-xs uppercase font-semibold rounded-full hover:bg-emerald-700 transition disabled:opacity-50 shadow-lg shadow-emerald-900/30">
            {scraping ? 'Scraping...' : 'Scrape mmareplayfull'}
          </button>
      ...
      {scrapeResult && (
        <div className="bg-emerald-900/20 border border-emerald-800/50 rounded-xl px-4 py-3 text-xs text-emerald-300">
          {scrapeResult}
        </div>
      )}
